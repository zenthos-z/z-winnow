...
def _g(**kw) -> GroupStatus:
    base = {
        ...
        "cron_human": "凌晨 2:00",
        "cron_valid": True,
        "next_fire": "2026-07-24T02:00:00+08:00",
        "last_run_date": "20260722",
        "last_run_at": "2026-07-22T02:00:00Z",
        "missing_days": 0,
    }
    base.update(kw)
...
def test_dashboard_renders_enabled_and_missing_cues():
    status = SchedulerStatus(
        ...
        tz="Asia/Shanghai",
        daemon_liveness="running",
        daemon_last_tick="2026-07-23T02:00:00+08:00",
    )
...
def test_dashboard_invalid_cron_marked():
    status = SchedulerStatus(
        groups=[_g(cron="garbage", cron_valid=False, cron_human="garbage", next_fire=None)]
    )
    out = views.render_to_str(views.build_dashboard(status))
...
def test_preflight_panel_ok_and_fail():
    ok = PreflightReport(
        [CheckResult("db", "ok", "data/x.db", critical=True), CheckResult("docker", "ok", "v27")]
    )
    out_ok = views.render_to_str(views.build_preflight_panel(ok))
    assert "全部就绪" in out_ok

    bad = PreflightReport(
        [CheckResult("memos_containers", "fail", "缺少 2/4", "docker compose up", critical=True)]
    )
...
def test_group_detail_invalid_warns():
    out = views.render_to_str(
        views.build_group_detail(_g(cron="bad", cron_valid=False, cron_human="bad"))
    )
    assert "无效" in out
